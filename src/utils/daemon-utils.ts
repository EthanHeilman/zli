...
const WAIT_UNTIL_USED_ON_HOST_RETRY_TIME = 100;
...
export async function handleServerStart(logPath: string, localPort: number, localHost: string) {
    await new Promise<void>(async (resolve, reject) => {
        await waitUntilUsedOnHost(localPort, localHost, WAIT_UNTIL_USED_ON_HOST_RETRY_TIME, WAIT_UNTIL_USED_ON_HOST_TIMEOUT).then(function () {
            resolve();
        }, function (err) {
