...
export async function listKubernetesPoliciesHandler(
    ...
    let clusterTargets = null;
    let environments = null;
    let kubernetesPolicies = null;

    // Create a promise to retrieve kube cluster targetrs
    const getClusterTargets = new Promise<KubeClusterSummary[]>( async (res) => {
        res(await kubeHttpService.ListKubeClusters());
    });
    ...
    // Create promise to retrieve kubernetes policies
    const getKubernetesPolicies = new Promise<KubernetesPolicySummary[]>( async (res) => {
        res(await policyHttpService.ListKubernetesPolicies());
    });

    // Await on all promises to make requests in parallel
    [ clusterTargets, environments, kubernetesPolicies ] = await Promise.all([getClusterTargets, getEnvironments, getKubernetesPolicies]);
    ...
    if(!! argv.json) {
        ...
        if (kubernetesPolicies.length === 0){
            ...
            return getTableOfKubernetesPolicies(kubernetesPolicies, userMap, apiKeyMap, environmentMap, targetNameMap, groupMap);
        }
    }
}